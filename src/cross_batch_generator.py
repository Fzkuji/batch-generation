...
class CrossBatchGeneratorWithHook:
    """
    Alternative implementation using forward hooks to inject cross-batch interaction
    into the model's forward pass without modifying the model itself.
    """
    ...
    def _cross_batch_hook(self, module, input, output):
        """Hook to inject cross-batch interaction."""
        ...
        if batch_size > 1 and seq_len > 0:
            # Only mix the last token's hidden state
            last_hidden = hidden_states[:, -1, :]
            mixed_hidden = self.cross_batch_module(
                last_hidden,
                attention_mask=self._valid_mask,
            )
    ...
    def _register_hook(self):
        """Register the cross-batch hook on target layer."""
        ...
        target_layer = layers[self.target_layer]
        self._hook_handle = target_layer.register_forward_hook(self._cross_batch_hook)

    def _remove_hook(self):
        """Remove the cross-batch hook."""
    ...
    def generate(
        ...
        **kwargs,
        ...
        """Generate with cross-batch interaction via hooks."""
        self._register_hook()
        self._is_generating = True
        self._valid_mask = torch.ones(input_ids.size(0), dtype=torch.bool, device=self.device)
